...
contains
    ...
    subroutine molecular_distance_comapare(arr1, ref1, ref2, n1, n2, internal0, xdim, distance)
        ...
        real(real64) :: R, rotarr1(3, n1), rotarr2(3, n2), arr0(3, n1 + n2), angles(6)
    ...
    end subroutine molecular_distance_comapare

    !input  is in cartesian
    !output is R and the 6 euler angles in zyz

    subroutine convert_isotopic_coordinates(cart, general_coordinates, &
                                            mass, mass0, &
                                            natom1, natom2, &
                                            ref1_0, ref2_0, &
                                            xdim, errors, do_test)
        ...
        real(real64), intent(inout) :: general_coordinates(7)
        real(real64), intent(inout) :: errors(4)
        integer(int32), optional :: do_test
        ...
        general_coordinates(1) = dsqrt(cm(1)**2 + cm(2)**2 + cm(3)**2)! "R"! Norm2

        theta = dacos(cm(3) / general_coordinates(1))! "TH"
        cos_theta = cm(3) / general_coordinates(1)
        ...
        general_coordinates(2) = alpha1
        general_coordinates(3) = beta1
        general_coordinates(4) = gamma1
        general_coordinates(5) = alpha2
        general_coordinates(6) = beta2
        general_coordinates(7) = gamma2
        ...
        if(present(do_test))then
            ...
            if (do_test>0)then
                ...
                call euler_angles_to_autosurf(xdim, general_coordinates, inter0_test)
                call  molecular_distance_comapare(tcf, cart_ref1, cart_ref2, natom1, natom2, &
                        inter0_test, xdim, distance_arr_test)
                td(1:2) = distance_arr_test
                call  molecular_distance_comapare(tci, cart_ref1, cart_ref2, natom1, natom2, &
                        inter0_test, xdim, distance_arr_test)
    ...
    subroutine get_pes_coordinates(pes_internal_coordinate, &
        ...
        real(real64) :: general_coordinates(7) ! general coordinate array in EUler-zyz
        ...
        if (present(errors))then
            call convert_isotopic_coordinates( cart, general_coordinates, &
                                                mass, mass0, &
                                                natom1, natom2, &
                                                ref1_temp0, ref2_temp0, &
                                                xdim, td, 1)
            errors = td
        else
            call convert_isotopic_coordinates( cart, general_coordinates, &
